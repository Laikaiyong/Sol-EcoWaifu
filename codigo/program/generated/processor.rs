...
use crate::generated::errors::NftProgramError;
use crate::generated::instructions::NftProgramInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = NftProgramInstruction::unpack(data)?;

        match instruction {
			NftProgramInstruction::Mint(args) => {
			...
			NftProgramInstruction::Transfer => {
				msg!("Instruction: Transfer");
				Self::process_transfer(program_id, accounts)
			}
			NftProgramInstruction::Burn => {
...
/// - short_description: [String] 
	pub fn process_mint(
		...
		if fee_payer_info.is_signer != true {
			return Err(NftProgramError::InvalidSignerPermission.into());
		}
		...
		if *gem_info.key != gem_pubkey {
			return Err(NftProgramError::NotExpectedAddress.into());
		}
		...
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(NftProgramError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != 82usize {
			return Err(NftProgramError::InvalidAccountLen.into());
		}
